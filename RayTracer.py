...
class Raytracer(object):
    ...
    def render(self, vision3D=False):
        fov = int(pi / 2)
        for y in range(self.height):
            for x in range(self.width):
                ...
                # Vision en 3D
                if (vision3D):
                    valEye = self.cast_ray(V3(0.4,0,0),direction)
                    valEye = self.cast_ray(V3(-0.4,0,0),direction)
                    if not valEye.equals(self.currentbg_color):
                        valEye = valEye*0.57 + color(100,0,0)
                    if not valEye.equals(self.currentbg_color):
                        valEye = valEye*0.57 + color(0,0,100)
                    eye_sum = valEye + valEye
    ...
    def show(self, filename='final.bmp', vision3D=False):
        self.render(vision3D)
        self.write(filename)
